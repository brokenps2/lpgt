...
void initNuklear() {
...

    {
        ...
        struct nk_font *termin = nk_font_atlas_add_from_file(atlas, res("fonts/Terminus-Bold.ttf"), 14, 0);
...
void updateNuklear() {
    ...
    if (nk_begin(ctx, "menu", nk_rect(x, y, w, h), NK_WINDOW_BORDER)) {
        ...
        if (nk_combo_begin_color(ctx, nk_rgb_cf(bg), nk_vec2(nk_widget_width(ctx),400))) {
            ...
            bg.b = nk_propertyf(ctx, "#B:", 0, bg.b, 1.0f, 0.01f,0.005f);                bg.a = nk_propertyf(ctx, "#A:", 0, bg.a, 1.0f, 0.01f,0.005f);
            nk_combo_end(ctx);
        }
    }
    nk_end(ctx);
